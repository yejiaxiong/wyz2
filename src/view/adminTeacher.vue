<template>
  <div>
    ...
    >
      ...
      <el-table-column prop='teacherName' label='姓名' width='90'> </el-table-column>

      <el-table-column prop='userName' label='用户名' width='110'>
      </el-table-column>
      <el-table-column prop='password' label='密码' width='110'> </el-table-column>
      <el-table-column prop='teacherJod' label='科目' width='110'> </el-table-column>
      <el-table-column prop='classId' label='授课班级' width='190'> </el-table-column>
      <el-table-column align='center' label="操作" width="">
        <template slot-scope='scope'>
          ...
          <el-button type='warning' size="small" @click='modTea(scope.row)'>修改</el-button>
        </template>
      </el-table-column>
    </el-table><br>
    <el-row >
...
export default {
  ...
  methods: {
    ...
    deleteRow(index, rows) {
      this.$confirm('此操作将永久删除该老师信息, 是否继续?', '提示', {
        ...
        let result = await this.$http({method: "POST",
          url: this.$http.defaults.baseURL+'deleteTeacher',
    ...
    // 修改老师信息 保存改行老师信息至sessionStorage中 跳转到modTeacher界面
    modTea(rows) {
    ...
    async getTeachers(){
      setTimeout(async ()=>{
        ...
        // console.log(typeof this.totalData[0].classId);

        for(let i=0;i<this.totalData.length;i++){
          this.totalData[i].classId = this.totalData[i].classId.join(",");
        }
